...
CEPH_DEV=1 ./bin/ceph tell osd.0 dump_metrics | tee $RESULT_DIR/result_${CURRENT_ROUND}_metrics.log > /dev/null
while [ $CURRENT_ROUND -lt $TARGET_ROUND ]
do
  (( ++CURRENT_ROUND ))
  echo "start round $CURRENT_ROUND ..."
  CEPH_DEV=1 ./bin/rados bench -p $POOL_NAME $BENCH_SECONDS write -b 4096 --no-cleanup | tee $RESULT_DIR/result_${CURRENT_ROUND}_bench.log
  CEPH_DEV=1 ./bin/ceph tell osd.0 dump_metrics | tee $RESULT_DIR/result_${CURRENT_ROUND}_metrics.log > /dev/null
